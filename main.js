...
const DrawSurface= () => {
    ...
    for (let j = x - SHOE_DIV / 2; j <= x + SHOE_DIV / 2; j+= 0.5) {
        // const aj = j * Math.PI / SHOE_DIV;
        // const sj = Math.sin(aj);
        // const cj = Math.cos(aj);
        for (let i = y - SHOE_DIV / 2; i <= y + SHOE_DIV / 2; i+= 0.5) {
            // const ai = i * 2 * Math.PI / SHOE_DIV;
            // const si = Math.sin(ai);
            // const ci = Math.cos(ai);

            // const multiplier = R + a * Math.cos(n * ai);

            const z = Math.pow(j, 3) / 3 - Math.pow(i, 2) / 2;

            vertices.push(j, i, z);  // X

            const s = j / SHOE_DIV * 2;
            const t = i / SHOE_DIV * 2;
            texCoords.push(s);
            texCoords.push(t);
        }
    }

    for (let j = 0; j < SHOE_DIV * 2; j++) {
        for (let i = 0; i < SHOE_DIV * 2; i++) {
            const p1 = j * (SHOE_DIV + 1) + i;
            const p2 = p1 + (SHOE_DIV + 1);
            indices.push(p1);
            indices.push(p2);
            indices.push(p1 + 1);
            indices.push(p1 + 1);
...
class StereoCamera {
    ...
    getFrustum = function (leftRight) {
        ...
        const translation = m4.translation((leftRight ? 1 : -1) * this.mEyeSeparation / 2 / 100, 0, 0);
...
function draw(){
    ...
    let translateToPointZero = m4.translation(0, 0, -distance);
    ...
    const modelViewProjectionL = m4.multiply(cam.getFrustum(true), matAccum1);
    const modelViewProjectionR = m4.multiply(cam.getFrustum(false), matAccum1);
 ...
 */
async function init() {
    xInput = document.getElementById('numX');
    xSlider = document.getElementById('x');
    yInput = document.getElementById('numY');
